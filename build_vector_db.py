...
def load_to_vector_db(df, db_path="./cocktails_vector_db"):
    ...
    # הוספנו את המרכיבים הנקיים ל-metadata כדי שנוכל לחשב "מרכיבים חסרים" בזמן אמת
    metadatas = df[['Cocktail Name', 'Glassware', 'Ingredients_Cleaned']].rename(
        columns={'Cocktail Name': 'name', 'Glassware': 'glassware', 'Ingredients_Cleaned': 'ingredients_raw'}
    ...
    # שומרים גם את ה-metadata יחד עם ה-bm25 כדי שתהיה לנו גישה מהירה לרשימת המרכיבים בלי לפנות ל-Chroma שוב
...
if __name__ == "__main__":
    # שימוש בקובץ נתונים אחד בלבד כפי שביקשת
    cocktails_file = "iba_live_scraped_data.csv" # תשנה לשם הקובץ האמיתי שלך
